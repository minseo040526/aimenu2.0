...
ALL_TAGS = sorted(uniq_tags(bakery_df) | uniq_tags(drink_df))
BAKERY_TAGS = sorted(uniq_tags(bakery_df))
DRINK_TAGS = sorted(uniq_tags(drink_df))

SWEETNESS_TAGS = {'달콤한','고소한','짭짤한','단백한','부드러운','깔끔한','쌉싸름한','상큼한','씁쓸한'}
UTILITY_TAGS = {'든든한','간단한','화려한'}

ui_sweetness_tags = sorted(t for t in (DRINK_TAGS | BAKERY_TAGS) if t in SWEETNESS_TAGS) or DRINK_TAGS or ALL_TAGS
ui_utility_tags = sorted(t for t in BAKERY_TAGS if t in UTILITY_TAGS) or BAKERY_TAGS
